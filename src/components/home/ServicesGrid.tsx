          ...
          {services.map((service, i) => {
            ...
            return (
              <Link 
                key={i} 
                href={service.href}
                className="group relative bg-navy rounded-xl p-8 transition-all duration-700 ease-out hover:-translate-y-2 hover:shadow-2xl flex flex-col items-start min-h-[360px] overflow-hidden"
              >
                {/* Background Image Output with specific opacity */}
                <Image 
                  src={service.image} 
                  fill 
                  className="object-cover opacity-60 group-hover:opacity-75 transition-all duration-700 ease-out group-hover:scale-105" 
                  alt={service.title} 
                />
                ...
                <div className="relative z-10 w-14 h-14 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center mb-6 text-white transition-transform duration-700 ease-out group-hover:scale-110">
                  <Icon weight="light" className="w-7 h-7" />
                </div>
                
                <h3 className="relative z-10 text-2xl font-heading text-white font-medium mt-auto mb-3 drop-shadow-sm group-hover:text-blush transition-colors duration-700 ease-out">
                  {service.title}
                </h3>
                ...
                <div className="relative z-10 text-white text-sm font-semibold tracking-wide uppercase flex items-center gap-2 group-hover:text-white pb-2">
                  <span className="border-b border-transparent group-hover:border-blush transition-colors duration-700 ease-out pb-1">Find out more</span>
                  <span className="transition-transform duration-700 ease-out group-hover:translate-x-1.5 text-blush">→</span>
                </div>
              </Link>
            )
          })}
